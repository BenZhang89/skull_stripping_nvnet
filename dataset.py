...
def flip_image(image, axis):
    try:
        ...
        # print(f'flip image shape is {new_data.shape}')
...
def offset_image(image, offset_factor):
    ...
    # print(f'offset image shape is {image_shape}')
...
#for whole three labels together, Ben
# def augment_image(image, flip_axis=None, offset_factor=None):
#     for l_idx in range(image.shape[0]):
#         if flip_axis is not None:
#             image[l_idx] = flip_image(image[l_idx], axis=flip_axis)
#         if offset_factor is not None:
#             image[l_idx] = offset_image(image[l_idx], offset_factor=offset_factor)
#
#     return image
...
#     """
#     ncr = label_data == 1  # Necrotic and Non-Enhancing Tumor (NCR/NET)
#     ed = label_data == 2  # Peritumoral Edema (ED)
#     et = label_data == 4  # GD-enhancing Tumor (ET)
#     target_label = np.array([ncr, ed, et], dtype=np.uint8)
#     # for l_idx in range(config["n_labels"]):
#     #     assert config["labels"][l_idx] in [1, 2, 4], "Wrong label!Expected 1 or 2 or 4, but got {0}".format(
#     #         config["labels"][l_idx])
#     #     if not config["label_containing"]:
#     #         target_label[np.where(label_data == config["labels"][l_idx])] = 1
#     #     else:
#     #         # #v1, whole, core, enhancing, has overlap: get always 1 in val acc.
#     #         # if config["labels"][l_idx] == 1:
#     #         #     target_label[0][np.where(label_data == 1)] = 1
#     #         #     target_label[0][np.where(label_data == 4)] = 1
#     #         # elif config["labels"][l_idx] == 2:
#     #         #     target_label[1][np.where(label_data > 0)] = 1
#     #         # elif config["labels"][l_idx] == 4:
#     #         #     target_label[2][np.where(label_data == 4)] = 1
#     #         #v2, whole, core, enhancing, has overlap: get always 1 in val acc.
#     #         if config["labels"][l_idx] == 1:
#     #             target_label[0][np.where(label_data == 1)] = 1
#     #             # target_label[0][np.where(label_data == 4)] = 1
#     #         elif config["labels"][l_idx] == 2:
#     #             target_label[1][np.where(label_data == 2)] = 1
#     #         elif config["labels"][l_idx] == 4:
#     #             target_label[2][np.where(label_data == 4)] = 1
#
#     return target_label
...
class BratsDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # dimessions of data
        n_dim = len(seg_label[0].shape) #for single label
        # n_dim = len(seg_label[0][0].shape) #for whole 3 labels. Ben
        ...
        for data_channel in range(input_data.shape[0]):
            ...
            # print(input_data[data_channel].shape)
        ...
        # # Transform ndarray segmentation label to Nifti1Image Ben
        # for i in range(seg_label.shape[0]):
        #     seg_image = nib.Nifti1Image(dataobj=seg_label[i][0], affine=self.affine)
        #     seg_label[i] = resample_to_img(augment_image(seg_image, flip_axis=flip_axis, offset_factor=offset_factor), seg_image, interpolation="nearest").get_data()[0]
        ...
        if self.config["VAE_enable"]:
            ...
            # seg_label = np.squeeze(seg_label, axis=1) # from (3,1,128,128,128) to (3,128,128,128) Ben
