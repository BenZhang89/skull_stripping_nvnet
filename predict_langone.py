...
from main import config,fetch_training_data_files
...
config["pred_data_file"] = os.path.abspath("langone_pre_brats_data.h5") # data file for prediction
...
#get the external dataset
config["dicom_file"] = '/gpfs/scratch/bz957/brats/data/langone/processed_2018/pre/*/*'
config["data_file"] = os.path.abspath("langone_pre_brats_data.h5")
config["training_file"] = os.path.abspath("langone_pre_training_ids.pkl")
config["validation_file"] = os.path.abspath("langone_pre_validation_ids.pkl")
config["test_file"] = os.path.abspath("langone_pre_test_ids.pkl")
...
def predict(model, model_prediction_dir, label):
    ...
    if config["overwrite"] or not os.path.exists(config["data_file"]):
        training_files, subject_ids = fetch_training_data_files(te_percentage = 1, return_subject_ids=True)
    ...
    try:
        ...
        for index in test_idxs:
            ...
            output_array = output_array > 0.5 #0.9
            ...
            if len(os.listdir(case_dir))>=3 and label==4:
                ...
                print(os.path.join(case_dir, "2018top1_seg_whole.nii.gz"))
                output_image_combined.to_filename(os.path.join(case_dir, "2018top1_seg_whole.nii.gz"))
...
if __name__ == "__main__":
    if config["checkpoint_file"] is not None:
        ...
        for i in range(len(config["best_model_files"])):
            ...
            model_prediction_dir = os.path.join(config["prediction_dir"],'whole_label_dice','pre')
